...
void draw_histogram(char top_fife_symbols[5], float percentage_of_occurrence[5], int histogram_width, int histogram_height, char filler)
{
    ...
    if (histogram_width >= minimal_width)
    {
        ...
        int count_of_steps_for_output_weight = calculate_count_of_steps_for_output_weight(height_weight_step, weight_of_percent, maximal_value_for_percent);
        ...
        for (int height_index = histogram_height - 1; height_index >= 1; height_index--)
        {
            ...
            for (int percentage_of_occurrence_index = 0; percentage_of_occurrence_index < 5; percentage_of_occurrence_index++)
            {
                if (percentage_of_occurrence[percentage_of_occurrence_index] >= value_for_percent)
                {
                    ...
                    for (int space_index = 1; space_index <= *(indents_between_indicators + indent_index); space_index++)
                    {
                        printf("%c", ' ');
                    }
                    indent_index++;
        ...
        for (int index_of_indicator = 1; index_of_indicator <= 5; index_of_indicator++)
        {
            ...
            for (int space_index = 1; space_index <= *(indents_between_indicators + index_of_indicator - 1); space_index++)
            {
                printf(" ");
            }
        }
